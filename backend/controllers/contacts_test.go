...
//
// TestGetContacts03 Test get contacts 02 - Test search
//
func TestGetContacts03(t *testing.T) {
	// Start the db connection.
	db, dbName, _ := models.NewTestDB("")
	defer models.TestingTearDown(db, dbName)
	...
	// Setup request
	req, _ := http.NewRequest("GET", "/api/v3/33/contacts?search=inc", nil)
	...
	st.Expect(t, results[0].Name, "Abc Inc.")
	st.Expect(t, results[1].Name, "Zoo Inc.")
}

//
// TestGetContacts04 Test get contacts 04 - Test search, no results
//
func TestGetContacts04(t *testing.T) {
	// Start the db connection.
	db, dbName, _ := models.NewTestDB("")
	defer models.TestingTearDown(db, dbName)
	...
	// Setup request
	req, _ := http.NewRequest("GET", "/api/v3/33/contacts?search=jsjsjjs", nil)
